...
class Game_Level_Selection:
    ...
    def start(self):
        self.game_selection_screen_is_active = True
        ...
        while self.game_selection_screen_is_active:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT or pygame.key.get_pressed()[pygame.K_ESCAPE]:
                    if self.VERBOSE: print 'Game selection canceled, returning to main menu'
                    return

            mouse_pos = pygame.mouse.get_pos()

            if self.back_button.check_click(mouse_pos[0], mouse_pos[1]):
                if self.VERBOSE: print 'Switching to game selection page %s' % str(self.selected_page)
            ...
            if self.next_button.check_click(mouse_pos[0], mouse_pos[1]):
                if self.VERBOSE: print 'Switching to game selection page %s' % str(self.selected_page + 2)
                self.selected_page += 1
            ...
            if button is not None:
                m.current_level = int(button.text) - 1
                if self.VERBOSE: print 'Game level selected: level %s' % button.text
                return Game(self.levels, self.update, self.VERBOSE, self.DEBUG, self.NOAI, self.NOAUDIO, self.SHOW_FPS).start(self.screen)
